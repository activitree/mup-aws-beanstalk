...
function tmpBuildPath(appPath, api) {
    ...
    return api.resolvePath(os_1.default.tmpdir(), `mup-meteor-${(0, uuid_1.v4)({ random: Uint8Array.of(...uuidNumbers) })}`);
}
...
async function getLogs(api, logNames) {
    ...
    const logsForServer = EnvironmentInfo && EnvironmentInfo.reduce((result, { Ec2InstanceId, Message }) => {
        if ((0, lodash_isstring_1.default)(Ec2InstanceId)) {
...
async function ensureRuleTargetExists(ruleName, target) {
    ...
    if (!Targets.find(_target => (0, lodash_isequal_1.default)(_target, target))) {
        const params = {
            Rule: ruleName,
            Targets: [target]
        };
