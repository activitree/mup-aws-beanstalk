...
export function tmpBuildPath(appPath, api) {
  ...
  return api.resolvePath(os.tmpdir(), `mup-meteor-${uuid.v4({
    random: uuidNumbers
  })}`);
}
...
export async function getLogs(api) {
  ...
  return Promise.all(Object.keys(logsForServer).map(key => new Promise((resolve, reject) => {
    axios.get(logsForServer[key]).then(({
      data
    }) => {
      resolve({
        data,
...
export function getAccountId() {
  return sts.getCallerIdentity().promise().then(({
    Account
  }) => Account);
}
...
export async function ensureCloudWatchRule(name, description, eventPattern) {
  ...
  try {
    await cloudWatchEvents.describeRule({
      Name: name
    }).promise();
  } catch (e) {
    error = true;
...
export function checkLongEnvSafe(currentConfig, commandHistory, appConfig) {
  ...
  const previouslyMigrated = currentConfig[0].OptionSettings.find(({
    Namespace,
    OptionName
  }) => Namespace === 'aws:elasticbeanstalk:application:environment' && OptionName === 'MUP_ENV_FILE_VERSION');
  const reconfigCount = commandHistory.filter(({
    name
  }) => name === 'beanstalk.reconfig').length;
  const ranDeploy = commandHistory.find(({
    name
  }) => name === 'beanstalk.deploy') && reconfigCount > 1;
  return {
...
export async function ensureSsmDocument(name, content) {
  ...
  try {
    const result = await ssm.getDocument({
      Name: name,
