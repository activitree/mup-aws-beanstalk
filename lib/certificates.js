...
export async function ensureSSLConfigured(config, certificateArn) {
    const { app, environment } = names(config);
    const ebConfig = [{
    ...
    const current = firstConfig.OptionSettings.reduce(convertToObject, {});
