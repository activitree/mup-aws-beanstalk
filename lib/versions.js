...
import { beanstalk, s3 } from './aws.js';
...
export async function s3Versions(api, providedPrefix) {
    const config = api.getConfig();
    const versions = [0];
    const { bucket, bundlePrefix } = names(config);
...
export async function largestEnvVersion(api) {
    ...
    const prefix = 'env/';
    const config = api.getConfig();
    const { bucket: bucketName } = names(config);
