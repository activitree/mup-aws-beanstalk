...
import { acm, s3, beanstalk, autoScaling, cloudTrail } from './aws.js';
import { ensureSSLConfigured } from './certificates.js';
import { rolePolicy, trailBucketPolicy, DeregisterEvent, deregisterEventTarget, serviceRole, eventTargetRolePolicy, eventTargetRole, gracefulShutdownAutomationDocument, passRolePolicy } from './policies.js';
import upload, { uploadEnvFile } from './upload.js';
import { archiveApp, injectFiles } from './prepare-bundle.js';
import { coloredStatusText, ensureBucketExists, ensureInstanceProfileExists, ensureRoleExists, ensureRoleAdded, ensurePoliciesAttached, ensureBucketPolicyAttached, getAccountId, getLogs, logStep, names, tmpBuildPath, shouldRebuild, ensureCloudWatchRule, ensureRuleTargetExists, ensureInlinePolicyAttached, findBucketWithPrefix, createUniqueName, createVersionDescription, ensureSsmDocument, selectPlatformArn, pickInstance, connectToInstance, executeSSHCommand } from './utils.js';
import { largestVersion, ebVersions, oldVersions, oldEnvVersions } from './versions.js';
import { createEnvFile } from './env-settings.js';
import { createDesiredConfig, prepareUpdateEnvironment, scalingConfig, getEnvTierConfig, scalingConfigChanged } from './eb-config.js';
import { waitForEnvReady, waitForHealth } from './env-ready.js';
import { startLogStreamListener, stopLogStreamListener } from "./deployment-logs.js";
export async function setup(api) {
    const config = api.getConfig();
    const appConfig = config.app;
    const { bucket: bucketName, app: appName, instanceProfile, serviceRole: serviceRoleName, trailBucketPrefix, trailName, deregisterRuleName, environment: environmentName, eventTargetRole: eventTargetRoleName, eventTargetPolicyName, eventTargetPassRoleName, automationDocument } = names(config);
    logStep('=> Setting up');
    ...
    if (appConfig.gracefulShutdown) {
        ...
        const createdDocument = await ensureSsmDocument(automationDocument, gracefulShutdownAutomationDocument());
        if (createdDocument) {
...
export async function deploy(api) {
    ...
    const config = api.getConfig();
    const { app, bucket, bundlePrefix, environment } = names(config);
    ...
    if (willBuild) {
        ...
        injectFiles(api, app, nextVersion, config.app);
    ...
    const { toRemove, toUpdate } = await prepareUpdateEnvironment(api);
    const eventLog = [];
    if (api.verbose) {
    ...
    const { Environments: finalEnvironments } = await beanstalk.describeEnvironments({
        ApplicationName: app,
        EnvironmentNames: [environment]
    });
...
export async function reconfig(api) {
    ...
    if (!environmentExists) {
        const desiredEbConfig = createDesiredConfig(api.getConfig(), api.getSettings(), config.app.longEnvVars ? 1 : false);
...
export async function events(api) {
    const { environment } = names(api.getConfig());
...
export async function status(api) {
    ...
    const { InstanceHealthList } = await beanstalk.describeInstancesHealth({
        AttributeNames: [
            'All'
        ],
        EnvironmentName: environment
...
export async function shell(api) {
    const { selected, description } = await pickInstance(api.getConfig(), api.getArgs()[2]);
    if (!selected) {
    ...
    const { sshOptions, removeSSHAccess } = await connectToInstance(api, selected, 'mup beanstalk shell');
...
export async function debug(api) {
    const config = api.getConfig();
    const { selected, description } = await pickInstance(config, api.getArgs()[2]);
    if (!selected) {
        console.log(description);
    ...
    const { sshOptions, removeSSHAccess } = await connectToInstance(api, selected, 'mup beanstalk debug');
    const conn = new Client();
    conn.on('ready', async () => {
        const result = await executeSSHCommand(conn, 'sudo pkill -USR1 -u webapp -n node || sudo pkill -USR1 -u nodejs -n node');
        if (api.verbose) {
