...
import { acm, s3, beanstalk, autoScaling, cloudTrail } from './aws';
import updateSSLConfig from './certificates';
import { rolePolicy, trailBucketPolicy, DeregisterEvent, deregisterEventTarget, serviceRole, eventTargetRolePolicy, eventTargetRole, gracefulShutdownAutomationDocument, passRolePolicy } from './policies';
import upload, { uploadEnvFile } from './upload';
import { archiveApp, injectFiles } from './prepare-bundle';
import { coloredStatusText, ensureBucketExists, ensureInstanceProfileExists, ensureRoleExists, ensureRoleAdded, ensurePoliciesAttached, ensureBucketPolicyAttached, getAccountId, getLogs, logStep, names, tmpBuildPath, shouldRebuild, ensureCloudWatchRule, ensureRuleTargetExists, ensureInlinePolicyAttached, findBucketWithPrefix, createUniqueName, checkLongEnvSafe, createVersionDescription, ensureSsmDocument } from './utils';
import { largestVersion, largestEnvVersion, ebVersions, oldVersions, oldEnvVersions } from './versions';
...
export async function setup(api) {
  ...
  logStep('=> Setting up'); // Create bucket if needed
  ...
  logStep('=> Ensuring IAM Roles and Instance Profiles are setup'); // Create role and instance profile

  await ensureRoleExists(instanceProfile, rolePolicy);
  await ensureInstanceProfileExists(config, instanceProfile);
  await ensurePoliciesAttached(config, instanceProfile, ['arn:aws:iam::aws:policy/AWSElasticBeanstalkWebTier', 'arn:aws:iam::aws:policy/AWSElasticBeanstalkMulticontainerDocker', 'arn:aws:iam::aws:policy/AWSElasticBeanstalkWorkerTier', ...(appConfig.gracefulShutdown ? ['arn:aws:iam::aws:policy/service-role/AmazonEC2RoleforSSM'] : [])]);
  await ensureRoleAdded(config, instanceProfile, instanceProfile); // Create role used by enhanced health

  await ensureRoleExists(serviceRoleName, serviceRole);
  await ensurePoliciesAttached(config, serviceRoleName, ['arn:aws:iam::aws:policy/service-role/AWSElasticBeanstalkEnhancedHealth', 'arn:aws:iam::aws:policy/service-role/AWSElasticBeanstalkService']);
  ...
  if (appConfig.gracefulShutdown) {
    ...
    const trailBucketName = existingBucket ? existingBucket.Name : createUniqueName(trailBucketPrefix);
...
export async function deploy(api) {
  ...
  config.app.buildOptions.buildLocation = config.app.buildOptions.buildLocation || tmpBuildPath(config.app.path, api);
...
export async function reconfig(api) {
  ...
  logStep('=> Configuring Beanstalk Environment'); // check if env exists
  ...
  if (!Environments.find(env => env.Status !== 'Terminated')) {
    ...
    const desiredEbConfig = createDesiredConfig(api.getConfig(), api.getSettings(), safeToReconfig ? nextEnvVersion : 0);
    const {
      toRemove,
      toUpdate
    } = diffConfig(ConfigurationSettings[0].OptionSettings, desiredEbConfig.OptionSettings);
